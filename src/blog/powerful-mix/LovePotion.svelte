	...
	onMount(() => {
		...
		const points = paths.map((path) => {
			...
			return Array(steps)
				.fill()
				.map((_, i, { length }) => {
					const { x, y } = path.getPointAtLength((totalLength / (length - 1)) * i);
					return { x, y };
				});
		});
		...
		const animate = () => {
			...
			for (let i = dropletsData.length - 1; i >= 0; i--) {
				...
				const indexProgress = Math.floor(easing(t) * points[indexPoints].length);
				const { x, y } = points[indexPoints][indexProgress];
				droplets.children[i].translate = { x, y };
				if (dropletsData[i].ticker >= dropletsData[i].cycle) {
					dropletsData = [...dropletsData.slice(0, i), ...dropletsData.slice(i + 1)];
