<script>
  ...
  /** @typedef {{ href: string, action: number | 'start' | 'goal', row: number, column: number, ltr: boolean }} Cell */
  /** @typedef { Cell[] } Board */
  ...
  let board = $state(getBoard({ columns, rows, leftToRight }));
  const { column, row } = /** @type Cell */ (
    // svelte-ignore state_referenced_locally
    board.find((d) => d.action === "start")
  );
  ...
  const handleMove = async (roll, direction = 1) => {
    ...
    const { ltr } = /** @type Cell */ (
      board.find((cell) => cell.column === column && cell.row === row)
    );
    ...
    const useDice = /** @type { SVGUseElement } */ (dice.querySelector("use"));

    useDice.setAttribute("href", `#dice-face-${roll}`);

    const animatePlayer = /** @type { SVGAnimateElement } */ (
      player.querySelector("animate")
    );

    animatePlayer.setAttribute("repeatCount", "2");
    animatePlayer.setAttribute("dur", `${duration / 1000 / 2}s`);
    animatePlayer.beginElement();
    ...
    useDice.setAttribute("href", `#dice-face-${Math.max(1, roll - 1)}`);
  ...
  const handleRoll = async () => {
    ...
    const animateTransform = /** @type { SVGAnimateTransformElement } */ (
      dice.querySelector("animateTransform")
    );
    const animate = /** @type { SVGAnimateElement } */ (
      dice.querySelector("animate")
    );
