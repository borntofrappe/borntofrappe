<script>
  ...
  const { Anchor, Ellipse, Shape } = pkg;
  ...
  onMount(() => {
    ...
    const label = /** @type { HTMLParagraphElement  } */ (
      widget.querySelector("p")
    );
    const buttonNext = /** @type { HTMLButtonElement } */ (
      widget.querySelector('button[data-action="next"]')
    );
    const buttonPrevious = /** @type { HTMLButtonElement } */ (
      widget.querySelector('button[data-action="prev"]')
    );
    ...
    const animateOut = () => {
      ...
      if (counter >= thresholds[1]) {
        ...
        if (direction === 1) {
          ...
          const delay = textContent ? textContent.length * 100 : 1000;
          timeout = setTimeout(() => {
