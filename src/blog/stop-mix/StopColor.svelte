<script>
  ...
  let stop = $state(stops[1]);
</script>

<div>
  <svg
    style="display: block;"
    style:--color-start={stop[0]}
    style:--color-end={stop[1]}
    viewBox="-19.5 -14.5 39 33"
  >
    <defs>
      <linearGradient id="stop-color-gradient">
        <stop
          style="stop-color: var(--color-start, yellow)"
          stop-color="yellow"
          offset="0.66"
        />
        <stop
          style="stop-color: var(--color-end, orange)"
          stop-color="orange"
          offset="0"
        />
      </linearGradient>
  ...
  <fieldset>
    <legend>Jersey colors</legend>
    {#each stops as value}
      <label>
        <input type="radio" name="stop-color-pair" {value} bind:group={stop} />
