<script>
  ...
  const { Illustration, Anchor, Group, Shape, Ellipse, RoundedRect, TAU } = pkg;

  /** @type {HTMLDivElement} */
  let element;
  onMount(() => {
    const element1 = /** @type {SVGSVGElement} */ (
      element.querySelector("svg:nth-of-type(1)")
    );
    const element2 = /** @type {SVGSVGElement} */ (
      element.querySelector("svg:nth-of-type(2)")
    );
