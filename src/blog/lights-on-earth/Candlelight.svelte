<script>
  ...
  const { Anchor, Shape, Ellipse, Cylinder } = pkg;
  ...
  onMount(() => {
    ...
    const spheres = Array(iterations)
      ...
      .map((_, i, { length }) => {
        const offset =
          offsets[0] + ((offsets[1] - offsets[0]) / (length - 1)) * i;
    ...
    candle.translate.z =
      coaster.stroke / 2 + candle.stroke / 2 + candle.length / 2;
    flame.translate.z =
      flame.stroke / 2 + candle.stroke / 2 + candle.length / 2;
