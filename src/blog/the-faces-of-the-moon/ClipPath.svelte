<script>
  /** @type {{ x: number, y: number, transformClip: boolean }}*/
  let { x = 0, y = 0, transformClip = true } = $props();

  let transform = $derived(`translate(${x} ${y})`);
  let target = $derived(transformClip ? "the clip" : "the face");
  let id = $derived(transformClip ? "clip-path-clip" : "clip-path-face");
</script>
