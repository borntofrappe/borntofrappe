<script>
  ...
  let value = $state(0);
  let linear = $derived(value * offset * -1);
  let step = $derived(Math.floor(value * steps) * jump * -1);
</script>
...
<div>
  <svg style="display: block;" viewBox="0 0 5.03 2.635">
    ...
    <svg width="5.03" height="1.285" viewBox="-2.5 -26 503 128.5">
      ...
      <use
        filter="url(#translate-filter)"
        opacity="0.5"
        href="#translate-region-linear"
      />
      <rect
        fill="currentColor"
        fill-opacity="0.1"
        ...
        height="100"
      />
      <text fill="currentColor" font-size="18" font-family="monospace" y="-8"
        >continuous</text
      >
    </svg>
