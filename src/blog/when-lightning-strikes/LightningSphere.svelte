  ...
  onMount(() => {
    ...
    const segments = [
      ...
      ...Array(lines)
        ...
        .map((_, i, { length }) => {
          const x =
            start + Math.floor(((end - start) / (length + 1)) * (i + 1));

          const max =
            offset1 + ((offset2 - offset1) / length) * (length - i - 1);
      ...
      const stroke =
        stroke1 + roundPercentage(((stroke2 - stroke1) / length) * i);
    ...
    }, /** @type Array<{color: string, stroke: number, path: Array<Point>}> */ ([]));
    ...
    const animate = () => {
      for (const lightning of storm) {
        ...
        if (lightning.counter > threshold) {
          const index = lightning.anchor.children.findIndex(
            (/** @type { any & { visible: boolean; }} */ child) =>
              child.visible === false
          );
          if (index === -1) {
            for (const child of lightning.anchor.children) {
    ...
    const handleKeydown = (event) => {
      if (event.key !== " " || state !== "wait") return;
  ...
  <button disabled aria-label="Press and hold to strike lightning">
    ...
    >
      ...
      >
        ...
        />
        <path
          d="M 11 67 C 20 59 18 46 20 39 22 12 60 7 74 23  M 80 31 C 89 50 82 78 75 92"
        />
        <path
