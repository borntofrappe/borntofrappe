<script>
  ...
  let luminance = $derived(
    color.reduce((acc, { value, weight }) => acc + (value / 255) * weight, 0)
  );
</script>
...
<div>
  ...
  <fieldset>
    {#each color as { name, label, value }, i}
      <label>
        <span style:--name={name}>{label}</span>
        <output>{value}</output>
        <input type="range" min="0" max="255" bind:value={color[i].value} />
      </label>
    {/each}
  </fieldset>
