<script>
	export let value = 0;
	export let accentColor = 'hsl(328, 85%, 46%)';
</script>

<svg viewBox="-50 -50 100 100">
	<g fill="none" stroke="#ffdb47" stroke-width="3" stroke-linecap="round">
		<path transform="rotate({value})" d="M 35 0 h 7" />
	</g>

	<circle r="28" fill="#ffdb47" />
	<g fill="#ff877a">
		<g transform="translate(0 5)">
			<circle r="4" cx="-9" />
			<circle r="4" cx="9" />
		</g>
		<g transform="translate(0 6)">
			<g stroke="#ff877a" stroke-width="2" stroke-linejoin="round" stroke-linecap="round">
				<path d="M -1.5 0 v 1.5 a 1.5 1.5 0 0 0 3 0 v -1.5z" />
			</g>
		</g>
	</g>

	<g fill="#38311e" stroke="#ffdb47" stroke-width="1">
		<circle r="2" cx="-5" />
		<circle r="2" cx="5" />
	</g>
</svg>

<form on:submit|preventDefault>
	<label>
		<span class="visually-hidden">Drag to rotate the segment around the center.</span>
		<input style:accent-color={accentColor} type="range" min={0} max={360} bind:value />
	</label>
</form>

<p>
	<strong style:border-bottom="0.2rem solid {accentColor}">{value}</strong> degrees
</p>

<style>
	svg {
		display: block;
	}

	input {
		inline-size: 100%;
		margin: 0.75em 0;
	}
</style>
