<script>
  ...
  let gradient = $state(gradients[0]);
</script>
...
<article>
  <svg style="display: block;" viewBox="0 0 34 34">
    <defs>
      <linearGradient id="reference-game-gradient">
        {#each gradient.stops as stop, i}
          <stop
            stop-color={stop}
            offset={(1 / (gradient.stops.length - 1)) * i}
          />
        {/each}
      </linearGradient>
      <linearGradient
    ...
    <rect
      fill="url(#reference-game-gradient-rotate)"
      y="4"
      width="2"
      height="30"
    />
    <circle
      fill="url(#reference-game-gradient-radial)"
      cx="20"
      cy="20"
      r="14"
    />
    ...
    <fieldset>
      {#each gradients as value}
        <label>
          ...
          <span
            style="background: linear-gradient(to right, {value.stops.join(
              ', '
            )})"
          ></span>
