<script>
  ...
  /** @type {{ childrenText: [string, string] }}*/
  let { childrenText = ["first", "second"] } = $props();

  /** @type {HTMLDivElement} */
  let widget;
  onMount(() => {
    const handleSwap = () => {
      const [first, second] = /** @type {[HTMLLIElement,HTMLLIElement]} */ ([
        ...widget.querySelectorAll("ul li"),
      ]);
      const parentNode = /** @type {HTMLUListElement} */ (second.parentNode);
      if (document.startViewTransition) {
        ...
        document.startViewTransition(() => {
          parentNode.insertBefore(second, first);
        });
      } else {
        parentNode.insertBefore(second, first);
      }
