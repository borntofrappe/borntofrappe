<script>
  /** @type {{ attributeName: 'opacity' | 'fill-opacity' }}*/
  let { attributeName = "fill-opacity" } = $props();

  /** @type { SVGSVGElement }*/
  let svg;

  const handleAnimation = async () => {
    const from = /** @type {  Array<SVGSetElement> } */ (
      [...svg.querySelectorAll("set.from")].slice(1)
    );
    const to = /** @type {  NodeListOf<SVGSetElement> } */ (
      svg.querySelectorAll("set.to")
    );
