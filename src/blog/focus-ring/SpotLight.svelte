<script>
	...
	const id = `spot-light-gradient-${colors.join('-')}`;
...
>
	...
	<circle fill={colors[0]} stroke-width="0.02" cx={fx} cy={fy} r="0.05" />
...
<!-- prettier-ignore -->
<pre><code>&lt;radialGradient cx="<span>{formatCoordinate(cx)}</span>" cy="<span>{formatCoordinate(cy)}</span>" r="<span>{formatCoordinate(r)}</span>" fx="{formatCoordinate(fx)}" fy="{formatCoordinate(fy)}" id="spotlight"&gt;
