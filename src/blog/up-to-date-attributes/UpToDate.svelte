<script>
  ...
  let widget;
  onMount(() => {
    if (!customElements.get("up-to-date")) {
      // svelte-ignore perf_avoid_nested_class
      class UpToDate extends HTMLElement {
        constructor() {
          super();
          ...
          button.addEventListener("click", () => {
            this.number = Math.floor(Math.random() * 100).toString();
          });
      ...
      customElements.define("up-to-date", UpToDate);
    }

    const customElement = /** @type { HTMLElement }*/ (
      widget.querySelector("up-to-date")
    );
  ...
  <svg style="display: block;" viewBox="0 0 100 46">
    ...
    <g font-family="monospace" font-weight="700">
      ...
      <g fill="hsl(0 0% 94%)" font-size="3.75">
        ...
        <text x="16" y="29"
          >number="<tspan data-key="number" fill="hsl(195 74% 71%)">5</tspan
          >"</text
        >
