<script>
  ...
  }, /** @type Array<{value: string, odds: number}> */ ([]));
  ...
  const getOptions = (n = 4) =>
    Array(n)
      ...
      .map((_, i) => {
        ...
        const vase = vases.find((vase) => odds < vase.odds);
        const value = vase ? vase.value : 0;
        return {
          value,
          i,
        };
  ...
  let options = $state(getOptions(n));
...
<div>
  ...
  <p>
    ...
    >
    {#key index}
      <span in:typewriter>{index ? options[index].value : ""}</span>
    {/key}
  </p>
</div>
