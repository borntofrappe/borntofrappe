<script>
  ...
  let column = $state(Math.floor(Math.random() * columns));
  let row = $state(Math.floor(Math.random() * (rows - 1)) + 1);
...
<article>
  ...
  >
    ...
    <rect
      style:cursor={mode === "find" ? "initial" : "zoom-in"}
      width={columns}
      height={rows}
  ...
  {#if mode === "play"}
    {#key feedback}
      <p
        in:fly={{ duration: 750, delay: 200, easing }}
