<script>
  ...
  let i = $state(snippets.length - 1);
  let snippet = $derived(snippets[i]);
  let code = $derived(highlight(snippet));
...
<article>
      ...
      <g transform="translate(5 5)">
        ...
        <g fill="hsl(130 27% 64%)">
          <path
            ...
            d="M 0 0 L 12 0 A 12 12 0 0 1 0 12"
          />
  ...
  <pre><code
      >{#each code as line (line)}<span
          style="display: block;"
          transition:slide={{ duration: 400 }}>{@html line}</span
        >{/each}</code
    ></pre>
</article>
