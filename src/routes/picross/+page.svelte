<script>
  ...
  /** @typedef {import('./levels.js').Key} Key */
  ...
  const entries = /** @type Array<[Key,Level]>*/ (Object.entries(levels));
  let value = $state(entries[0][0]);
</script>
...
<main>
  <div class="[ visually-hidden ]">
    <h1>Picross</h1>
    <p>
      Explore solved nonogram puzzles with crisp visuals and in multiple
      dimensions.
    </p>
  </div>
  <label>
...
<style>
  main {
    ...
    background-image: url('data:image/svg+xml;utf8,<svg viewBox="-5 -5 10 10"\ xmlns="http://www.w3.org/2000/svg"><circle r="0.5" fill="hsl(43 13% 90%)" /></svg>');
  ...
  select {
    ...
    transition:
      outline var(--transition-duration, 0.25s) 0s
        cubic-bezier(0.37, 0, 0.63, 1),
      box-shadow var(--transition-duration, 0.25s) 0s
        cubic-bezier(0.37, 0, 0.63, 1);
  }
