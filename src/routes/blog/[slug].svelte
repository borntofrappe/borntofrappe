<script context="module">
	...
	export async function load({ page, session }) {
		...
		if (post) {
			...
			const isLatest = session.posts.findIndex(d => d.slug === page.params.slug) === 0;
			return {
				props: {
					...metadata,
					Module,
...
<header class:isLatest>
	<h1>{title}</h1>
	{#if tags}
		<h2 class="visually-hidden">Tags</h2>
		<ul>
			{#each tags.split(/,\s?/) as tag}
				<li>
