export default {
	mouse: [
		'transform: translate(20px, 40px) rotate(0deg);',
		'transform: translate(-30px, -10px) rotate(0deg);',
		'transform: translate(70px, 90px) rotate(-90deg);',
		'transform: translate(-5px, 15px) rotate(90deg);',
		'transform: translate(70px, 90px) rotate(180deg);',
		'transform: translate(20px, 40px) rotate(180deg);',
		'transform: translate(120px, 15px) rotate(-45deg); transform-origin: 0px 25px;'
	],
	heart: [
		'transform: translate(125px, 35px) rotate(180deg);',
		'transform: translate(75px, 85px) rotate(90deg);',
		'transform: translate(25px, 35px) rotate(0deg);',
		'transform: translate(50px, 10px) rotate(180deg);',
		'transform: translate(25px, 35px) rotate(360deg);',
		'transform: translate(0px, 60px) rotate(180deg);',
		'transform: translate(25px, 10px) rotate(90deg);'
	],
	boat: [
		'transform: translate(55px, -35px) rotate(90deg);',
		'transform: translate(125px, 65px) rotate(135deg);',
		'transform: translate(25px, 65px) rotate(-45deg);',
		'transform: translate(90px, 30px) rotate(135deg);',
		'transform: translate(55px, 65px) rotate(180deg);',
		'transform: translate(90px, 30px) rotate(135deg);',
		'transform: translate(0px, 65px) rotate(45deg); transform-origin: 25px 0px;'
	],
	rocket: [
		'transform: translate(85px, 65px) rotate(-135deg);',
		'transform: translate(105px, 101px) rotate(135deg);',
		'transform: translate(85px, -5px) rotate(135deg);',
		'transform: translate(35px, 66px) rotate(45deg);',
		'transform: translate(15px, -5px) rotate(405deg);',
		'transform: translate(70px, 101px) rotate(315deg);',
		'transform: translate(-40px, 66px) rotate(-45deg); transform-origin: 75px 0px;'
	],
	butterfly: [
		'transform: translate(-50px, 50px) rotate(45deg); transform-origin: 100px 0px;',
		'transform: translate(50px, 85px) rotate(315deg);',
		'transform: translate(-20px, 50px) rotate(-45deg);',
		'transform: translate(50px, 50px) rotate(45deg);',
		'transform: translate(50px, 50px) rotate(315deg);',
		'transform: translate(85px, 85px) rotate(225deg);',
		'transform: translate(-25px, 50px) rotate(-45deg); transform-origin: 75px 0px;'
	],
	house: [
		'transform: translate(100px, 100px) rotate(180deg);',
		'transform: translate(0px, 50px) rotate(0deg);',
		'transform: translate(100px, 100px) rotate(180deg);',
		'transform: translate(60px, -10px) rotate(-45deg); transform-origin: 25px 25px;',
		'transform: translate(85px, 50px) rotate(180deg);',
		'transform: translate(50px, 50px) rotate(90deg);',
		'transform: translate(35px, 25px) rotate(45deg); transform-origin: 50px 25px;'
	],
	swan: [
		'transform: translate(50px, 45px) rotate(0deg);',
		'transform: translate(85.5px, 9.5px) rotate(45deg);',
		'transform: translate(0px, 95px) rotate(-135deg); ',
		'transform: translate(60.5px, 20px) rotate(45deg); transform-origin: 25px 25px;',
		'transform: translate(100px, 95px) rotate(180deg);',
		'transform: translate(0px, -25px) rotate(45deg);',
		'transform: translate(0px, -50px) rotate(90deg); transform-origin: 0px 25px;'
	],
	rabbit: [
		'transform: translate(65px, 75px) rotate(135deg);',
		'transform: translate(65px, 50px) rotate(0deg);',
		'transform: translate(-6px, 145px) rotate(-90deg);',
		'transform: translate(50px, -20px) rotate(-45deg); transform-origin: 50px 0px;',
		'transform: translate(-6px, 75px) rotate(315deg);',
		'transform: translate(79px, 145px) rotate(135deg);',
		'transform: translate(75px, -45px) rotate(-135deg); transform-origin: 0px 25px;'
	],
	fish: [
		'transform: translate(55px, -15px) rotate(45deg);',
		'transform: translate(55px, 0px) rotate(45deg);',
		'transform: translate(-15px, 70px) rotate(225deg);',
		'transform: translate(20px, 35px) rotate(45deg); transform-origin: 0px 0px;',
		'transform: translate(126px, 56px) rotate(135deg);',
		'transform: translate(20px, 70px) rotate(-45deg);',
		'transform: translate(20px, 10px) rotate(135deg); transform-origin: 0px 25px;'
	],
	goat: [
		'transform: translate(55px, 50px) rotate(-225deg);',
		'transform: translate(-15px, 75px) rotate(180deg);',
		'transform: translate(140px, 65px) rotate(-180deg);',
		'transform: translate(115px, 40px) rotate(180deg); transform-origin: 0px 0px;',
		'transform: translate(115px, 90px) rotate(180deg);',
		'transform: translate(65px, 90px) rotate(-45deg);',
		'transform: translate(140px, -10px) rotate(-135deg); transform-origin: 0px 25px;'
	],
	horse: [
		'transform: translate(50px, 15px) rotate(90deg);',
		'transform: translate(95px, 40px) rotate(-90deg);',
		'transform: translate(120px, -20px) rotate(-225deg);',
		'transform: translate(25px, -10px) rotate(45deg); transform-origin: 25px 25px;',
		'transform: translate(120px, 15px) rotate(135deg);',
		'transform: translate(85px, 135px) rotate(135deg);',
		'transform: translate(0px, 40px) rotate(-225deg); transform-origin: 0px 25px;'
	],
	serpent: [
		'transform: translate(150px, 50px) rotate(180deg);',
		'transform: translate(-35px, 0px) rotate(45deg);',
		'transform: translate(0px, 0px) rotate(0deg);',
		'transform: translate(0px, 0px) rotate(135deg);',
		'transform: translate(0px, 50px) rotate(360deg);',
		'transform: translate(-35px, 0px) rotate(90deg);',
		'transform: translate(115px, 25px) rotate(45deg); transform-origin: 0px 25px;'
	],
	scorpion: [
		'transform: translate(75px, 65px) rotate(180deg);',
		'transform: translate(10px, 30px) rotate(225deg);',
		'transform: translate(90px, 35px) rotate(0deg);',
		'transform: translate(60px, -5px) rotate(0deg);',
		'transform: translate(90px, 135px) rotate(225deg);',
		'transform: translate(-5px, 100px) rotate(225deg);',
		'transform: translate(10px, -30px) rotate(0deg); transform-origin: 0px 25px;'
	]
};
