<script>
	...
	onMount(() => {
		...
		for (let i = 0; i < length; i++) {
			ticks[i].setAttribute('stroke-dasharray', '1');
			ticks[i].setAttribute('stroke-dashoffset', '1');
		...
		const timeout = setTimeout(() => {
			for (const tick of ticks) {
				tick.setAttribute('stroke-dashoffset', '0');
			}
...
<svg bind:this={svg} viewBox="0 0 {5 * ticks.length} 5">
	<defs>
		{#each Array(4) as _, i}
			<g id="mark-{i}">
