<script>
  ...
  const dCircle = ({ radius = 20, points = 20, noise = 1 } = {}) => {
    ...
    return [...coords, { x, y }].reduce(
      (acc, { x, y }, i) =>
        i % 2 === 0 ? `${acc} ${x} ${y}` : `${acc} Q ${x} ${y}`,
      "M"
    );
  };

  /**
   * @param {{ x1: number, y1: number, x2: number, y2: number, noise: number, points?: number }} param
   */
  const dLine = ({ x1 = 0, y1 = 0, x2 = 10, y2 = 10, noise = 1, points }) => {
    const p =
      points || Math.floor(((x1 - x2) ** 2 + (y1 - y2) ** 2) ** 0.5 / 2);
...
<svg style="display: block;" viewBox="0 0 80 50">
  ...
  <g>
    ...
    <use
      href="#poke-the-ladybug-rock"
      transform="translate(65 40) scale(-1 1)"
    />
  </g>
