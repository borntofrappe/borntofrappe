<script>
  ...
  const scriptEnemy = [
    ...Array(hpEnemy - 1)
      .fill("")
      .map((_) => "You hit for 1 point!"),
    "You defeated the enemy!",
  ].map((message, i, { length }) => {
    const id = `fightTheEnemyHitEnemy${i}`;
    const initial =
      i === 0 ? `${start}.begin` : `fightTheEnemyHitEnemy${i - 1}.end`;
  ...
  /** @typedef {{ character: string, begin: string }} Character */
  const scriptYou = [
    ...Array(hpYou - 1)
      .fill("")
      .map((_) => "You are hit for 1 point!"),
    "You were defeated...",
  ].reduce((acc, curr, i) => {
    ...
    if (acc.length > 0) {
      const match = /** @type { RegExpMatchArray } */ (
        acc[acc.length - 1].begin.match(/([\d.]+)s$/)
      );
      offset = parseFloat(match[1]) + delay;
    }
  ...
  }, /** @type Array<{id: string, begin: string, show: string, dismiss: string, characters: Character[], hp: number}> */ ([]));
          ...
          >
            <text>
              ...
              {#each characters as { character, begin }}
                <tspan
                  >{character}<set attributeName="fill-opacity" to="0" /><set
                    {begin}
