<script>
  ...
  const getX = () =>
    padding + Math.floor(Math.random() * (width - size - padding * 2));
  const getY = () =>
    padding + Math.floor(Math.random() * (height - size - padding * 2));
  ...
  const deck = cards.reduce((acc, card, i) => {
    ...
    return [...acc, ...pair];
  }, /** @type Array<{x: number, y: number, hrefs: string[], id: string, match: string, resets: string[]}> */ ([]));
</script>
