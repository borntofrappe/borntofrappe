<script>
  ...
  /** @type Array<{fill: string, stroke: string, copies: Array<{x: number, y: number, width: number, height: number}>}> */
  const clouds = [
    {
      fill: "#f2f2f2",
      stroke: "#c5c9cc",
  ...
  while (true) {
    const cloudWidth =
      cloudWidthMin + randomUpTo(cloudWidthMax - cloudWidthMin);
  ...
  const durations = {
    ...
    targets: [
      Math.floor(targetTranslateX / 12) + randomUpTo(4),
      Math.floor(targetTranslateX / 12) + randomUpTo(4),
    ],
  };

  const overlayX = targetWidth / 2.5;
  ...
  const { targets } = durations.targets.reduce(
    ...
    {
      targets:
        /** @type Array<{i: number, y: number, duration: number, delay: number}> */ ([]),
      delay: 0,
    }
  );
...
<svg style="display: block;" viewBox="0 0 80 50">
  ...
  <defs>
    ...
    <symbol id="time-the-shots-spaceship" viewBox="-19 -11 38 25">
      <g stroke="currentColor" stroke-linejoin="round" stroke-linecap="round">
        <g fill="currentColor">
          ...
          <path
            transform="scale(-1 1)"
            d="M -18 9 a 25 25 0 0 1 16 -10 a 10 10 0 0 0 -16 10"
          />
        </g>
        <g fill="#fec749">
    ...
    <symbol id="time-the-shots-target-0" viewBox="-18 -18 36 25">
      ...
      >
        ...
        <path
          fill="#5589a3"
          d="M -9 -9.5 a 17 6.25 0 1 0 18 0 30 30 0 0 1 -18 0"
        />
        <path
          fill="#3dbfb3"
          d="M -9 -9.5 a 9.1 9.1 0 0 1 18 0 30 30 0 0 1 -18 0"
        />
      </g>
    </symbol>
      ...
      <g transform="translate(0 {y})">
        <g>
          ...
          <use
            width={targetWidth}
            height={targetHeight}
            href="#time-the-shots-target-0"
          >
            <animate
              id="timeTheShotsTarget{i}Outro"
  ...
  <g>
    ...
    <set begin="timeTheShotsFinish.begin" attributeName="display" to="none" />
  ...
  <g transform="translate({targets.length * 80 * -1} 0)">
    ...
    {#each targets as { i }}
      <animateTransform
        begin="timeTheShotsTarget{i}Outro.end"
        attributeName="transform"
        type="translate"
        by="80 0"
        dur="0.1"
    ...
    <g>
      ...
      <AnimatedTitle
        ...
        begin="timeTheShotsTarget0Outro.end"
        end="timeTheShotsFinish.begin"
        repeatCount="indefinite"
      />
    </g>
  ...
  <g style:cursor="pointer">
    ...
    <Title fill="url(#linear-gradient-text)">Shoot down!</Title>
