<script>
	...
	const colors = {
		canvas: 'hsl(37 23% 93%)',
		easel: 'hsl(20 76% 84%)',
		paint: ['hsl(203 51% 68%)', 'hsl(37 71% 69%)', 'hsl(9 67% 65%)', 'hsl(138 18% 67%)']
	};
	...
	onMount(() => {
		...
		const getRadius = (min = 20, max = 40) => min + Math.floor(Math.random() * (max - min));
		...
		const handleEnd = () => {
			...
			state = 'wait';
			color = colors.paint[(colors.paint.indexOf(color) + 1) % colors.paint.length];
		};
...
<style>
	...
	div::after {
		...
		background-image: linear-gradient(hsl(0 0% 0% / 0.1), hsl(0 0% 0% / 0.02) 40%),
